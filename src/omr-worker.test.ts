...
describe("OMR worker LH chord-completion post-pass (issue #113)", () => {
  it("defines the additive post-pass function", () => {
    expect(worker).toContain("def complete_lh_chords(");
  });

  it("calls the post-pass in process_job before put_object", () => {
    const passCall = worker.indexOf("body = complete_lh_chords(body)");
    expect(passCall, "complete_lh_chords must be invoked").toBeGreaterThan(-1);
    const putObject = worker.indexOf("client.put_object(");
    expect(putObject).toBeGreaterThan(-1);
    // The completion must run BEFORE the upload so R2 gets the completed XML.
    expect(passCall).toBeLessThan(putObject);
  });

  it("runs the post-pass only on the engine-success path, never on the sentinel", () => {
    // The call sits inside the `result_path is not None` branch (engine output),
    // not in the else branch that writes FAILURE_SENTINEL.
    const successBranch = worker.indexOf("if result_path is not None:");
    const elseBranch = worker.indexOf("body = FAILURE_SENTINEL");
    const passCall = worker.indexOf("body = complete_lh_chords(body)");
    expect(successBranch).toBeGreaterThan(-1);
    expect(elseBranch).toBeGreaterThan(-1);
    expect(passCall).toBeGreaterThan(successBranch);
    expect(passCall).toBeLessThan(elseBranch);
  });

  it("scopes the pass to staff 2 and chord shapes (LH only, additive)", () => {
    // It keys off the bass staff and the <chord> element, and never raises into
    // process_job (self-guarding try/except returns the input bytes).
    expect(worker).toMatch(/LH_STAFF\s*=\s*"2"/);
    expect(worker).toContain('find("chord")');
    expect(worker).toContain("never raise into process_job");
  });

  it("uses no em or en dashes in the worker source (post-pass)", () => {
    expect(worker).not.toMatch(/[–—]/);
  });
});
